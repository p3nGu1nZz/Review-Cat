...
- Eliminating human oversight entirely (humans may still choose to review or
  approve; the system should not *require* human involvement to keep running).
...
- Requires `GITHUB_PERSONAL_ACCESS_TOKEN` via environment (non-interactive).
  If unset, it should exit with a clear message rather than prompting.
